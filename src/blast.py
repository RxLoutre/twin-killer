...
"""
Script created by a.zielezinski (Biostars)
"""
...
class Hsp:
    ...
    def __str__(self):
        f = "{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}\t{11}".format(self.qid, self.sid, self.pident, self.length, self.mismatch, self.gaps, self.qstart, self.qend, self.sstart, self.send, self.evalue, self.bitscore)
        return f
